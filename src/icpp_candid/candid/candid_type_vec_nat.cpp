...
CandidTypeVecNat::CandidTypeVecNat() : CandidTypeBase() {
...
void CandidTypeVecNat::push_back_value(CandidTypeRoot &value) {
  auto &derived_value = static_cast<CandidTypeNat &>(value);
  if (m_pv) {
    // We're deserializing
...
// ---------
// Initialize things
void CandidTypeVecNat::initialize() {
  set_datatype();
  set_content_type();
  encode_T();
  encode_I();
...
void CandidTypeVecNat::set_datatype() {
...
bool CandidTypeVecNat::decode_T(VecBytes B, __uint128_t &offset,
...
void CandidTypeVecNat::encode_I() {
