...
CandidTypeVecFloat64::CandidTypeVecFloat64() : CandidTypeBase() {
  std::vector<double> v;
  set_v(v);
  initialize();
}
...
void CandidTypeVecFloat64::push_back_value(CandidTypeRoot &value) {
  auto &derived_value = static_cast<CandidTypeFloat64 &>(value);
  if (m_pv) {
    // We're deserializing
...
// ---------
// Initialize things
void CandidTypeVecFloat64::initialize() {
  set_datatype();
  set_content_type();
  encode_T();
  encode_I();
...
void CandidTypeVecFloat64::set_datatype() {
...
bool CandidTypeVecFloat64::decode_T(VecBytes B, __uint128_t &offset,
                                    std::string &parse_error) {
...
void CandidTypeVecFloat64::encode_I() {
